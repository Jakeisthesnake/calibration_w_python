...
def generate_synthetic_cameras():
    ...
    dist1 = np.array([-0.2, -0.05, 0.02, -0.01]) 
    dist2 = np.array([-0.2, -0.05, 0.02, -0.01])  
    dist3 = np.array([-0.2, -0.05, 0.02, -0.01]) 
...
def generate_synthetic_data(K_1, dist_coeffs_1, K_2, dist_coeffs_2, K_3, dist_coeffs_3, R1, t1, R2, t2, R3, t3, obj_points, img_size, rpy_range, xyz_range, num_samples, output_dir, board_size):
    ...
    angles = np.radians(np.linspace(*xyz_range['t'], 10))
    radius_values = [1, 2]
    y_values = [-0.6]
...
rty_range = {'r': (1, 2), 't': (-30, 180), 'y': (-0.6, 0.1)}  # Meters
