...
def process_csv(file_path, target_cam_id):
    ...
    grouped_data = defaultdict(lambda: {'obj_points': [], 'img_points': [], 'camera_rotation': [], 'camera_translation': [], 'object_rotation': [], 'object_translation': []})
    # print("che1")
    timestamp_last = None
    # print("df.iterrows()\n", df.iterrows())
    for _, row in df.iterrows():
        ...
        if timestamp != timestamp_last:
            cam_rotation = np.array([row['R_cam_00'], row['R_cam_01'], row['R_cam_02'],
                                    row['R_cam_10'], row['R_cam_11'], row['R_cam_12'],
                                    row['R_cam_20'], row['R_cam_21'], row['R_cam_22']], dtype=np.float32).reshape(3, 3)
            cam_translation = np.array([row['t_cam_x'], row['t_cam_y'], row['t_cam_z']], dtype=np.float32)
            object_rotation = np.array([row['R_obj_00'], row['R_obj_01'], row['R_obj_02'],
                                    row['R_obj_10'], row['R_obj_11'], row['R_obj_12'],
                                    row['R_obj_20'], row['R_obj_21'], row['R_obj_22']], dtype=np.float32).reshape(3, 3)
            object_translation = np.array([row['t_obj_x'], row['t_obj_y'], row['t_obj_z']], dtype=np.float32)
            grouped_data[timestamp]['camera_rotation'] = cam_rotation
            grouped_data[timestamp]['camera_translation'] = cam_translation
            grouped_data[timestamp]['object_rotation'] = object_rotation
            grouped_data[timestamp]['object_translation'] = object_translation
    ...
    camera_rotations = [np.array(data['camera_rotation'], dtype=np.float32) for data in grouped_data.values()]
    camera_translations = [np.array(data['camera_translation'], dtype=np.float32) for data in grouped_data.values()]
    object_rotations = [np.array(data['object_rotation'], dtype=np.float32) for data in grouped_data.values()]
    object_translations = [np.array(data['object_translation'], dtype=np.float32) for data in grouped_data.values()]
    # print("obj_pts:\n", obj_pts_list)
    
    return obj_pts_list, img_pts_list, camera_rotations, camera_translations, object_rotations, object_translations
...
def kannala_brandt_project(points, K, dist_coeffs):
    ...
    for X, Y, Z in points:
        ...
        ac1, ac2, ac3, ac4 = [0.1, -0.05, 0.02, -0.01]
        theta_real = theta + ac1 * theta**3 + ac2 * theta**5 + ac3 * theta**7 + ac4 * theta**9
        theta_real = theta 
        
        theta_d = theta + k1 * theta**3 + k2 * theta**5 + k3 * theta**7 + k4 * theta**9
        # scale = theta_d / r if r > 1e-8 else 1.0
        scale = theta_real / r if r > 1e-8 else 1.0
        
        x_distorted, y_distorted = X * scale, Y * scale
        u, v = fx * x_distorted + cx, fy * y_distorted + cy
        projected_points.append([u, v])
        theta_diff = theta_real - theta
    
    return np.array(projected_points), theta_diff
...
def fisheye_reprojection_error(params, obj_pts_list, img_pts_list, num_images):
    ...
    for i in range(num_images):
        ...
        theta_diffs.append(theta_i)

    print("total_error\n", np.sum(np.hstack(total_error)))
    print("average theta_diff\n", np.mean(theta_diffs))
    # print("max theta\n", np.max(all_theta))
...
def calibrate_fisheye_camera(data_file):
    ...
    """
    obj_pts_list, img_pts_list, camera_poses, camera_transforms, object_poses, object_transforms = process_csv(data_file, target_cam_id=0)
    print("Checkpoint\n")
    # print("obj_pts_list\n", obj_pts_list)
    # print("img_pts_list\n", img_pts_list)
